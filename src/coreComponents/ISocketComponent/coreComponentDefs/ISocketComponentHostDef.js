/**
 * @def ISocketComponent
 * @isGroup true
 * 
 * @CSSify styleName : ISocketComponentHost/false
 * @CSSify styleName : ISocketComponentTemplate/false
...
var ISocketComponentDef = function(uniqueID, options, model) {
	...
	var slotDef = TypeManager.createComponentDef({
		host : TypeManager.createComponentDef({
			...
			]/**@CSSifyStyle componentStyle : ISocketComponentTemplate */
		}, null, 'hostOnly')
	}, null, 'rootOnly');
	 ...
	 * 
	 * This def is the base-def for any ISocketComponent instance
	 ...
	 * 		=> we have to take into account that there is a !second! def obj, which is injected directly into the ISocketComponent ctor
	 * 		=> so the ISocketComponent must then be extended through prototypal inheritance, and :
...
//			type : 'ComposedCompnent', 				// this is implicit, as we call the CompoundComponent ctor in the TabPanel ctor
			nodeName : 'ISocketComponent'.toLowerCase() + '-component'/**@CSSifyStyle componentStyle : ISocketComponentHost */
		}),
		lists : [
...
module.exports = ISocketComponentDef;